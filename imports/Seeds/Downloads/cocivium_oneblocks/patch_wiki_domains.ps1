...
$content = if (Test-Path "Domains.md") { Get-Content "Domains.md" -Raw } else { "# Domains`r`n" }
$link = "- [Finance & Budgeting](https://github.com/$Owner/$Repo/blob/main/domains/finance.md)"
if ($content -notmatch [regex]::Escape($link)) { $content = $content.TrimEnd() + "`r`n`r`n$link`r`n" }
Set-Content "Domains.md" $content -Encoding UTF8

git add Domains.md
git commit -m "wiki: link to Finance & Budgeting brief" | Out-Null
git push | Out-Null
