...
# A) Rebuild docs/tooltip-hotfix from origin/main (force update)
git switch -C docs/tooltip-hotfix origin/main | Out-Null
Repair-Readme
git add README.md
git commit -m "docs: fix README hover acronyms; remove stray delegate text" 2>$null
...
# B) Reset validator branch to remote, drop README change, fix CI
...
git commit -m "resolve: drop root README change; unify CI (hygiene + validator)" 2>$null
git push | Out-Null
$prAI = gh pr list --state open --json headRefName,number -q '.[]|select(.headRefName=="ai-registry/validator-skeleton")|.number'

# C) Merge PRs (relax, merge, restore)
...
Write-Host "`n✅ Fixed: tooltip PR rebased on main; validator PR cleaned; CI unified; protections restored." -ForegroundColor Green
