...
function Ensure-Path([string]$p){ if(!(Test-Path $p)){ New-Item -ItemType Directory -Force -Path $p | Out-Null } }
...
Ensure-Path $docsDir; Ensure-Path $hooksDir

# ----- Minimal docs (indented examples, no markdown code fences) -----
$THIN_SHIMS = @'
...
# ----- Pre-push hook (bash shim + PowerShell logic) -----
$PREPUSH_BASH = @'
#!/usr/bin/env bash
set -euo pipefail
...
# Normalize LF for bash shim
(Get-Content (Join-Path $hooksDir "pre-push")) -join "`n" | Set-Content -NoNewline (Join-Path $hooksDir "pre-push")
...
if(-not $SkipPR){
  if(Get-Command gh -ErrorAction SilentlyContinue){
    gh pr create --fill --title "BPOE: Thin Shims + Blob Guard + Branch Hygiene + ISSUEOPS refresh" `
      --body "Centralized CI docs, pre-push blob guard, branch hygiene notes, IssueOps quicksheet."
  } else {
