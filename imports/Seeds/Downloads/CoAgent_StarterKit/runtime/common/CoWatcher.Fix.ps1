...
if (-not (Test-Path $script:CoRoot)) { New-Item -ItemType Directory -Force -Path $script:CoRoot | Out-Null }
...
$null = New-Item -ItemType Directory -Force -Path $script:SessionRoot,$script:Inbox,$script:Outbox,$script:Logs | Out-Null
...
function Start-CoQueueWatcher {
  ...
  do {
    ...
    foreach($it in $items){
      ...
      try { $out = & $path 2>&1 | Out-String; $result='pass' }
        ...
        $evt | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $logJs -Encoding UTF8
      }
...
function Start-CoQueueWatcherJob {
  [CmdletBinding()]
  param([int]$PollMs=600)
  Start-Job -Name ("CoQueueWatcher-{0}" -f $env:COSESSION_ID) -ScriptBlock {
    Set-StrictMode -Version Latest; $ErrorActionPreference='Stop'
    . (Join-Path $env:USERPROFILE 'Downloads\CoTemp\Join-CoAgent.ps1')
    Start-CoQueueWatcher -Loop -PollMs $using:PollMs
  } | Out-Null
  Write-Host "Watcher job started." -ForegroundColor Cyan
}
