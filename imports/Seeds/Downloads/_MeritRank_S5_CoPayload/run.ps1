...
$reportPath = Join-Path $RepoRoot 'out\MeritRank_Local_Report_S5.md'
$lines = @('# MeritRank Local Report (Sprint 5)','',"**$score**",'','Components:','```'); $lines += $exps; $lines += @('```','','Detail JSON:','```',$detail,'```','','Badge JSON:','```',$badge,'```')
[IO.File]::WriteAllLines($reportPath,$lines,[Text.UTF8Encoding]::new($true)); Write-Host "Wrote $reportPath"; Get-Content $reportPath