...
foreach ($f in $files) {
  ...
  if (-not $obj.id)    { $errors += "Missing id in $($f.Name)" }
  if (-not $obj.name)  { $errors += "Missing name in $($f.Name)" }
  if ($obj.points -eq $null -or -not ($obj.points -is [double] -or $obj.points -is [int])) {
