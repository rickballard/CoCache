...
$cap = $config.caps
if ($cap) {
  foreach ($k in $cap.PSObject.Properties.Name) {
    $mx = [double]$cap.$k
    if ($norm.ContainsKey($k) -and $mx -gt 0) { $norm[$k] = [math]::Min([double]$norm[$k], $mx) }
  }
}
...
# Explanations
$exp = @()
foreach ($k in @('readme_length','ps1_count','md_count','registry_points')) {
  if ($components.ContainsKey($k)) {
    $exp += "{0}: +{1:N4} (norm {2:N3} × w {3:N2})" -f $k, $components[$k], $norm[$k], $w[$k]
  }
}
