...
git switch -C ops/index-and-bizplan origin/main 2>$null | Out-Null

# --- persist robust builder (single-quoted: no expansion while writing) ---
New-Item -ItemType Directory -Force -Path tools | Out-Null
Set-Content -Encoding UTF8 tools/build-index.ps1 @'
param()
Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"
...
foreach($f in $mds){
  ...
  $norm = @(@($norm) | Sort-Object -Unique)   # ALWAYS array
  $forward[$rel]=$norm
  foreach($n in $norm){
