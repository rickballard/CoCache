# DO — Refresh article and helper assets into repo, branch, PR
param([string]$Owner='rickballard',[string]$Repo='CoCivium')
Set-StrictMode -Version Latest; $ErrorActionPreference='Stop'
$RepoRoot = Join-Path $HOME "Documents\GitHub\$Repo"
$RelPath='insights'
$MdPath = Join-Path $RepoRoot "$RelPath\global-village-buyback.md"
$null = New-Item -ItemType Directory -Force -Path (Join-Path $RepoRoot "$RelPath\images"),(Join-Path $RepoRoot "$RelPath\audio")
...
try {
  $branch = "docs/gvb-refresh-$(Get-Date -Format 'yyyyMMddHHmm')"
  git checkout -b $branch
  git add insights
  git commit -m "docs: refresh Global Village Buyback and helper assets"
