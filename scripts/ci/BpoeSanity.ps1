...
$hits = Get-ChildItem -Recurse -File |
  Where-Object { $_.FullName -notmatch "\\.git\\" -and $_.FullName -notmatch "\\.github\\actions\\runner"  -and Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"
...
.FullName -notmatch "scripts\\ci\\BpoeSanity\.ps1" } | Select-String -SimpleMatch -Pattern "<branch>" -List
if ($hits) { $errors += "Found literal `<branch>` placeholder in repo: " + ($hits.Path -join ", ") }
