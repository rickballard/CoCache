...
# Utils
function New-Slug($s){ $t=($s -replace '[^\p{L}\p{Nd}]+' , '-').ToLower(); $t=($t -replace '-+','-').Trim('-'); if($t.Length -gt 60){$t=$t.Substring(0,[Math]::Min(60,$t.Length)).Trim('-')} return $t }
...
function Classify-ByName($name){ $n=$name.ToLower(); if($n -like 'cowrap*' -or $n -match '(?:^|_)cowrap' -or $n -match '(^|_)wrap($|_)'){return 'cowrap'} if($n -match '(^|_)plan($|_)' -or $n -like '*_plan_*'){return 'cowrap'} if($n -like 'ideacard*'){return 'ideacard'} 'ideacard' }
...
foreach($file in $allFiles){
  try{
    $src=$file.FullName; $ext=$file.Extension.ToLower()
    if($ext -in '.png','.jpg','.jpeg','.gif','.pdf'){
      Copy-Item -Path $src -Destination $attachDir -Force
      $rows.Add(("asset`t{0}`t{1}`t{2}" -f $file.Name,$src,(Join-Path $attachDir $file.Name))); continue
    }
    if($ext -in '.odt','.docx'){
      $kind=Classify-ByName $file.Name
      Copy-Item -Path $src -Destination $attachDir -Force
      $title=[IO.Path]::GetFileNameWithoutExtension($file.Name); $slug=New-Slug $title
...
owner: rick
created: $today
source: "CardsWraps.zip/session_$Stamp"
---

> Attachment captured from zip. See: `../attachments/session_$Stamp/$($file.Name)`
"@ | Set-Content -Encoding UTF8 $dest
      if($kind -eq 'cowrap'){$cwraps++}else{$cards++}
      $rows.Add(("{0}`t{1}`t{2}`t{3}" -f $kind,$file.Name,$src,$dest)); continue
    }
    if($ext -in '.md','.txt'){
      ...
      $slug=New-Slug $title
      $dest=if($kind -eq 'cowrap'){Join-Path $cwDir ("${datedPref}${slug}.md")}else{Join-Path $icDir ("${datedPref}${slug}.md")}
      if(Has-FrontMatter $raw){ $raw | Set-Content -Encoding UTF8 $dest }
      else{
        if($kind -eq 'cowrap'){
@"
---
