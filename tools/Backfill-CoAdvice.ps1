param(
  ...
  [switch]$DoIngest,       # also run tools/DO-AdviceBomb-Merge.ps1 per family (chronological)
  [switch]$NoDryRun,       # actually write inbox bundles (default = dry-run)
  [string[]]$Families,     # optional: only include these family stems (case-insensitive)
  [string[]]$Exclude       # optional: exclude these family stems
)
...
foreach($r in $searchRoots){
  $candidateFiles += Get-ChildItem -File -Recurse $r -ErrorAction SilentlyContinue |
    Where-Object {
...
$work = $plan.GetEnumerator() | ForEach-Object { $_.Value } | Sort-Object Family, StampDT, Version
if($Families){ $famSet = $Families | ForEach-Object { $_.ToLower() }; $work = $work | Where-Object { $famSet -contains $_.Family.ToLower() } }
if($Exclude){  $excSet = $Exclude  | ForEach-Object { $_.ToLower() }; $work = $work | Where-Object { $excSet -notcontains $_.Family.ToLower() } }
...
## Source Files
$(($w.Files | ForEach-Object { "- " + ($_.FullName.Substring((Resolve-Path $Root).Path.Length).TrimStart("\") ) }) -join "`r`n"))
...
if($created.Count){
  ...
  $created | ForEach-Object { P (" - {0} (v{1}, {2})" -f $_.Family,$_.Version,$_.Stamp) "Gray" }
}else{
