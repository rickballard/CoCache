...
$roots = @("advice","docs\intent\advice","docs\intent","docs") | % { Join-Path $Root $_ } | ? { Test-Path $_ } | Select-Object -Unique
$cands = @()
foreach($r in $roots){
  $cands += Get-ChildItem -File -Recurse $r -EA SilentlyContinue |
    ? { $_.FullName -notmatch "\\(node_modules|\.git)\\" -and $_.Extension -in ".zip",".md",".txt",".json" }
}
$recs = @()
